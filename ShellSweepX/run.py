...
class Settings(BaseModel):
    ...
    yara_rules: dict = {}
    ai_prompt: str = "You are an AI assistant tasked with analyzing potential webshells. Please analyze the following code and provide insights on whether it might be a webshell, its capabilities, and any suspicious elements."
...
async def read_root(request: Request):
    with get_db_connection() as conn:
        ...
        detection_rate = (webshells_detected / files_scanned * 100) if files_scanned > 0 else 0
        ...
        """)
        recent_detections = [{"file_name": row[0], "detected_time": row[1]} for row in c.fetchall()]
        ...
        c.execute("""
            ...
            ORDER BY month
        """)
        formatted_trend_data = [{"month": row[0], "count": row[1]} for row in c.fetchall()]
...
async def agents_page(request: Request):
    ...
    return templates.TemplateResponse("agents.html", {"request": request, "settings": settings})
...
async def settings_page(request: Request):
    ...
    return templates.TemplateResponse("settings.html", {"request": request, "settings": settings, "yara_rules": yara_rules, "message": message})
...
async def save_settings_route(request: Request):
    try:
        ...
        return RedirectResponse(url="/settings?message=Error saving settings: {str(e)}", status_code=303)

# Load the model and vectorizer
clf = joblib.load('models/model.pkl')
vectorizer = joblib.load('models/vectorizer.pkl')

def insert_into_db(sha256, file_name, result, file_size, content, entropy, std_dev, yara_matches, submitting_agent):
    with get_db_connection() as conn:
        c = conn.cursor()
        ...
        c.execute('''INSERT OR REPLACE INTO findings 
                     ...
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)''', 
                  (sha256, file_name, result, file_size, content, entropy, std_dev, yara_matches, current_time, current_time, current_time, submitting_agent))

def get_findings_from_db(limit=None, offset=0):
    with get_db_connection() as conn:
        c = conn.cursor()
        if limit:
            c.execute("SELECT sha256, file_name, result, file_size, analysis, entropy, std_dev FROM findings ORDER BY id DESC LIMIT ? OFFSET ?", (limit, offset))
        else:
            c.execute("SELECT sha256, file_name, result, file_size, analysis, entropy, std_dev FROM findings ORDER BY id DESC")
        findings = c.fetchall()
    return findings
...
def search_findings(query, limit=10, offset=0):
    with get_db_connection() as conn:
        ...
        if query:
            ...
            """, (limit, offset))
        findings = c.fetchall()
    return [{"sha256": f[0], "file_name": f[1], "result": f[2], "file_size": f[3], "content": f[4], "analysis": f[5], "entropy": f[6], "std_dev": f[7], "yara_matches": f[8], "submitted_timestamp": f[9], "last_analyzed_timestamp": f[10], "submitting_agents": f[11]} for f in findings]
...
async def upload_files(files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        insert_into_db(sha256, file.filename, result, file_size, content_str, entropy, std_dev, ','.join(yara_matches), "Manual Upload")
...
@app.get("/search")
async def search(query: str = "", page: int = Query(1, ge=1), page_size: int = Query(10, ge=1, le=100)):
...
def get_agents():
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute('SELECT agent_id, computer_name, last_checkin FROM agents ORDER BY last_checkin DESC')
        agents = [{"agent_id": row[0], "computer_name": row[1], "last_checkin": row[2]} for row in c.fetchall()]
    return agents
...
def get_active_agent_count():
    with get_db_connection() as conn:
        ...
        c.execute("SELECT COUNT(DISTINCT computer_name) FROM agents WHERE last_checkin > ?", (twenty_four_hours_ago.isoformat(),))
        count = c.fetchone()[0]
    return count
...
async def receive_agent_results(request: Request):
    try:
        ...
        for result in results:
            ...
            file_content = base64.b64decode(result['Content']).decode('utf-8', errors='ignore')
            ...
            insert_into_db(sha256, file_path, prediction_result, file_size, file_content, entropy, std_dev, ','.join(yara_matches), computer_name)
...
async def scan_database_with_yara():
    ...
    yara_rules = load_yara_rules()
    valid_rules = {name: rule_info for name, rule_info in yara_rules.items() if rule_info['is_valid']}
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("SELECT id, content FROM findings WHERE yara_matches IS NULL OR yara_matches = ''")
        findings = c.fetchall()
        ...
        for finding_id, content in findings:
            try:
                ...
                if yara_matches:
                    c.execute("UPDATE findings SET yara_matches = ? WHERE id = ?", (','.join(yara_matches), finding_id))
                    updated_count += 1
            except Exception as e:
                errors.append(f"Error processing finding {finding_id}: {str(e)}")

    log_message = f"YARA scan complete. Updated {updated_count} findings with YARA matches."
    if errors:
...
async def triage_with_ai(sha256: str = Form(...)):
    ...
    if settings.gpt_api_key:
        ...
        analysis = analyze_with_claude(chunks, settings.claude_api_key, settings.ai_prompt)
    else:
    ...
    with get_db_connection() as conn:
        c = conn.cursor()
        c.execute("UPDATE findings SET analysis = ?, last_analyzed_timestamp = ? WHERE sha256 = ?", (analysis, current_time, sha256))
...
async def add_yara_rule(request: Request):
    try:
        ...
        if file_path.exists():
            return JSONResponse({"message": "A rule with this name already exists."}, status_code=400)
        
        try:
            yara.compile(source=content)
        except yara.Error as e:
            return JSONResponse({"message": f"Invalid YARA rule: {str(e)}"}, status_code=400)
...
async def get_recent_detections():
    with get_db_connection() as conn:
        ...
        c.execute("""
            ...
            ORDER BY created_at DESC LIMIT 5
        """)
        recent_detections = [{"file_name": row[0], "detected_time": row[1], "severity": get_severity(row[2])} for row in c.fetchall()]
